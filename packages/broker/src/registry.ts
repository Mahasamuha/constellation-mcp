...
/** Registers a connection for the agent, returning any prior connection that occupied the slot
 * (the caller is responsible for terminating it — replacing a stale connection is normal on reconnect). */
export function registerConnection(conn: ConnectedAgent): ConnectedAgent | undefined {
  const existing = connections.get(conn.agentId);
  connections.set(conn.agentId, conn);
  return existing;
}
...
export function unregisterConnection(conn: ConnectedAgent): boolean {
  if (connections.get(conn.agentId) !== conn) return false;
...
export function allConnections(): IterableIterator<[string, ConnectedAgent]> {
...
// RPC registry
//
// Tracks in-flight requests awaiting a response from an agent. Isolated for
...
export function dispatchPendingRpc(requestId: string, agentId: string, timeoutMs: number): Promise<RpcResponse> {
 ...
 * timed out, or the response is for a request this broker no longer tracks). */
...
/** Rejects and removes every pending RPC belonging to the given agent — called on disconnect. */
export function rejectPendingRpcsForAgent(agentId: string, error: Error): void {
