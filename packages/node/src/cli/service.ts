...
/**
 * Registers the node with the OS service manager for user-scoped autostart.
...
function installSystemd(_exec: string): void {
  ...
  const unit = `[Unit]
Description=Constellation Node
After=network.target

[Service]
Type=simple
ExecStart=${execLine} node start --foreground
Restart=on-failure
RestartSec=5
...
function installLaunchd(_exec: string): void {
  ...
  const label = `com.constellation.node`;
...
${programArgs}
  ...
  <string>${join(homedir(), "Library", "Logs", "constellation-node.log")}</string>
...
function installTaskScheduler(_exec: string): void {
  ...
  const args = prefix.slice(1).concat(["node", "start", "--foreground"]).join(" ");
...
export function startService(): void {
  ...
  else if (p === "darwin") runInherited("launchctl", ["load", launchNodePlist()]);
...
export function stopService(): void {
  ...
  else if (p === "darwin") runInherited("launchctl", ["unload", launchNodePlist()]);
...
export function restartService(): void {
  ...
  if (p === "linux") runInherited("systemctl", ["--user", "restart", SERVICE_NAME]);
    ...
    runInherited("launchctl", ["unload", launchNodePlist()]);
    runInherited("launchctl", ["load", launchNodePlist()]);
  } else if (p === "win32") {
...
export function serviceStatus(): string {
  ...
  try {
    ...
    if (p === "darwin") {
      const out = run("launchctl", ["list", "com.constellation.node"]);
...
export function showLogs(follow: boolean, lines: number): void {
  ...
  if (p === "linux") {
    ...
    const logPath = join(homedir(), "Library", "Logs", "constellation-node.log");
...
function launchNodePlist(): string {
  return join(homedir(), "Library", "LaunchAgents", "com.constellation.node.plist");
}
