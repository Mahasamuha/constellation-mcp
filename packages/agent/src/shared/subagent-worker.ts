/**
 * Subagent worker — runs under the target user's OS identity.
 *
 * Started by the shared agent parent process via child_process.fork().
 ...
 *
 * IPC protocol:
 *   Parent → Worker:  SubagentInit (once), then SubagentRequest (many)
 *   Worker → Parent:  SubagentReady (once after init), then SubagentResponse
 */

import { AgentExecutor } from "../executor/index.js";
import { createLogger } from "@constellation/shared";

const log = createLogger("agent:subagent-worker");
...
if (!process.send) {
  process.stderr.write("subagent-worker: must be run as a forked child process\n");
  process.exit(1);
}

if (process.platform !== "linux") {
  process.stderr.write("subagent-worker: shared agent requires Linux\n");
  process.exit(1);
}
...
if (!targetUser || isNaN(targetUid) || isNaN(targetGid)) {
  process.stderr.write("subagent-worker: missing CONSTELLATION_TARGET_USER/UID/GID\n");
  process.exit(1);
}

if (targetUid === 0) {
  process.stderr.write("subagent-worker: refusing to run as root (uid 0)\n");
  process.exit(1);
}
...
try {
  ...
  process.stderr.write(`subagent-worker: failed to drop privileges: ${(err as Error).message}\n`);
  process.exit(1);
}

log.info({ username: targetUser, uid: targetUid, gid: targetGid }, "Subagent worker started");
...
function send(msg: SubagentReady | SubagentResponse): void {
  process.send!(msg);
}
...
process.on("message", (rawMsg: unknown) => {
  ...
  const msg = rawMsg as SubagentInit | SubagentRequest;

  if (msg.type === "init") {
    if (initialized) fatal("Received duplicate init message");
    executor = new AgentExecutor(msg.labels, msg.max_file_size_kb);
    initialized = true;
    send({ type: "ready" });
    return;
  }
...
function isValidMessage(msg: unknown): msg is SubagentInit | SubagentRequest {
