...
describe("resolveIdentity", () => {
  ...
  it("returns a generic identity error when no tier resolves", async () => {
    ...
    expect(isIdentityError(result)).toBe(true);
    if (isIdentityError(result)) {
      expect(result.message).toBe("Could not resolve an OS identity for this user. Contact the shared agent administrator.");
    }
  });

  it("never falls through to an agent-supplied identity outside the three tiers", async () => {
