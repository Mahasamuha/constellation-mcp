...
describe("listDirectory", () => {
  ...
  it("lists recursively", async () => {
    ...
    const result = await listDirectory(root, { recursive: true });
  ...
  it("respects max_depth", async () => {
    ...
    const result = await listDirectory(root, { recursive: true, max_depth: 1 });
    const paths = result.nodes.map((n) => n.path);
  ...
  it("excludes matching names", async () => {
    ...
    const result = await listDirectory(root, { exclude: ["node_modules"] });
    const paths = result.nodes.map((n) => n.path);
  ...
  it("truncates at limit and sets truncated_by=limit", async () => {
    ...
    const result = await listDirectory(root, { limit: 3 });
  ...
  it("limit:0 uses hard cap (does not error on normal-sized dirs)", async () => {
    await fs.writeFile(join(root, "a.txt"), "");
    const result = await listDirectory(root, { limit: 0 });
  ...
  it("scopes to relative_path subdirectory", async () => {
    ...
    const result = await listDirectory(root, { relative_path: "sub" });
    const paths = result.nodes.map((n) => n.path);
...
describe("fileInfo", () => {
  it("returns correct metadata for a regular file", async () => {
    ...
    const info = await fileInfo(root, "hello.txt");
  ...
  it("returns type:directory for a directory", async () => {
    ...
    const info = await fileInfo(root, "mydir");
  ...
  it("returns type:symlink and target for a symlink", async () => {
    ...
    const info = await fileInfo(root, "link.txt");
    expect(info.type).toBe("symlink");
    expect(info.target).toBe(join(root, "real.txt"));
  });

  it("throws when path does not exist", async () => {
    await expect(fileInfo(root, "nope.txt")).rejects.toThrow();
  });
});
...
describe("findFiles", () => {
  it("matches files by glob pattern", async () => {
    ...
    const result = await findFiles(root, { pattern: "*.ts" });
  ...
  it("matches files by regex pattern", async () => {
    ...
    const result = await findFiles(root, { pattern: "\\.test\\.", type: "regex" });
  ...
  it("scopes search to relative_path", async () => {
    ...
    const result = await findFiles(root, { pattern: "*.ts", relative_path: "src" });
  ...
  it("returns no matches for unmatched pattern", async () => {
    await fs.writeFile(join(root, "a.ts"), "");
    const result = await findFiles(root, { pattern: "*.py" });
...
describe("grepFiles", () => {
  it("finds literal matches across files", async () => {
    ...
    const result = await grepFiles(root, { pattern: "hello" });
  ...
  it("finds regex matches", async () => {
    ...
    const result = await grepFiles(root, { pattern: "^(const|let)", type: "regex" });
  ...
  it("scopes search with file_glob", async () => {
    ...
    const result = await grepFiles(root, { pattern: "TARGET", file_glob: "*.ts" });
  ...
  it("searches a single file when relative_path is a file", async () => {
    ...
    const result = await grepFiles(root, {
      pattern: "found it",
  ...
  it("truncates at 50 matches", async () => {
    ...
    const result = await grepFiles(root, { pattern: "MATCH" });
  ...
  it("groups matches by file", async () => {
    ...
    const result = await grepFiles(root, { pattern: "hit" });
    expect(result.results).toHaveLength(1);
...
describe("writeFile", () => {
  it("creates a new file with content", async () => {
    await writeFile(root, { relative_path: "new.txt", content: "hello" });
    expect(await fs.readFile(join(root, "new.txt"), "utf8")).toBe("hello");
  });

  it("overwrites existing file content", async () => {
    await fs.writeFile(join(root, "existing.txt"), "old");
    await writeFile(root, { relative_path: "existing.txt", content: "new" });
  ...
  it("creates parent directories on write", async () => {
    await writeFile(root, { relative_path: "deep/nested/file.txt", content: "x" });
...
describe("editFile", () => {
  ...
  it("dry_run returns diff without writing", async () => {
    ...
    const result = await editFile(root, {
      relative_path: "f.txt",
...
describe("copyPath", () => {
  it("copies a file", async () => {
    ...
    await copyPath(root, { src_relative_path: "src.txt", dst_relative_path: "dst.txt" });
  ...
  it("copies a directory recursively", async () => {
    ...
    await copyPath(root, { src_relative_path: "srcdir", dst_relative_path: "dstdir" });
...
describe("movePath", () => {
  it("moves a file", async () => {
    ...
    await movePath(root, { src_relative_path: "src.txt", dst_relative_path: "dst.txt" });
  ...
  it("creates destination parent directories", async () => {
    ...
    await movePath(root, {
      ...
      dst_relative_path: join("deep", "nested", "file.txt"),
    });
    expect(
...
describe("createDirectory", () => {
  ...
  it("creates nested directories", async () => {
    await createDirectory(root, join("a", "b", "c"));
  ...
  it("does not throw if directory already exists", async () => {
    ...
    await expect(createDirectory(root, "exists")).resolves.toBeUndefined();
  });
});
...
describe("deletePath", () => {
  it("deletes a file", async () => {
    ...
    await deletePath(root, { relative_path: "del.txt" });
  ...
  it("returns confirmation summary for directory without recursive flag", async () => {
    ...
    const result = await deletePath(root, { relative_path: "mydir" });
  ...
  it("deletes directory with recursive:true", async () => {
    ...
    await deletePath(root, { relative_path: "mydir", recursive: true });
