...
export default function Auth() {
  ...
  async function startFlow() {
    ...
    setError("");
    try {
      const info = await invoke<DeviceCodeInfo>("start_device_flow", { relayUrl: relayUrl.trim() });
      setCodeInfo(info);
      setStep("code");
  ...
  return (
    <div className="auth-container">
      {step === "url" && (
        <>
          <h2 className="auth-heading">Connect to Relay</h2>
          <label className="auth-label">Relay URL</label>
          <input
            ...
            onKeyDown={(e) => e.key === "Enter" && relayUrl && startFlow()}
            autoFocus
          />
          {error && <p className="auth-error">{error}</p>}
          <button
            className="auth-btn"
            style={{ opacity: !relayUrl || loading ? 0.5 : 1 }}
            disabled={!relayUrl || loading}
            onClick={startFlow}
          >
      ...
      {step === "done" && (
        <>
          ...
          <p className="auth-hint">Node registered successfully.</p>
        </>
      )}
