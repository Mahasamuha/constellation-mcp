...
pub async fn start_device_flow(relay_url: String) -> Result<DeviceCodeInfo, String> {
    let url = format!("{}/oauth/device/code", relay_url.trim_end_matches('/'));
    ...
    if !res.status().is_success() {
        return Err(format!("Relay error: {}", res.text().await.unwrap_or_default()));
    }
...
pub async fn poll_device_flow(
    ...
    let url = format!("{}/oauth/token", relay_url.trim_end_matches('/'));
    ...
    loop {
        ...
        if let Some(err) = body.error {
            match err.as_str() {
                ...
                // relay asking us to back off — continue at the same interval
...
fn write_config(dir: &Path, relay_url: &str, token: &str, host: &str) -> Result<(), String> {
    ...
    if let serde_yaml::Value::Mapping(ref mut m) = map {
        ...
        m.insert("node_token".into(), token.into());
