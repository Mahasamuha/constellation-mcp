...
| Field | Description |
|---|---|
| `share` | Unique name for this path across all agents on your account. Used as the routing key in MCP tool calls. |
...
Shares must be unique per account — two agents on the same account cannot use the same share name.
...
| `--json` | `executors list`, `shares list`, `filters list`, `sessions list`, `users list` | Emit machine-readable JSON output. |
| `--executor <id>` | `shares list`, `filters add` | Filter results or scope a filter to a specific executor ID. |
