# ADR 0013: Hub Config Changes Require Process Restart (No Live Reload)
...
On restart:
- Incoming RPCs are rejected immediately with `HUB_RESTARTING — retry after 45 seconds`.
...
On shutdown (SIGTERM):
- Incoming RPCs are rejected immediately with `HUB_SHUTTING_DOWN`.
- In-flight requests drain with a 30-second timeout.
- Pooled subnodes with no in-flight work receive SIGTERM immediately.
- Subnodes with in-flight work receive SIGTERM after their current response is sent or
...
The operational cost is low: hub config changes are infrequent (admin-driven,
...
- `constellation hub validate-config --config <path>` provides a dry-run
...
- Subnode pool state is lost on restart. All active subnode processes are terminated
