...
The node needs to tell the relay which path labels it has registered so the relay
can route MCP tool calls to the correct node. This creates two possible sources of
truth for the label registry: the relay's Postgres store, or the node's local
`paths.yaml`.

## Decision

The node's local `paths.yaml` is the sole authoritative source for label
definitions. The relay stores a copy of label→path mappings in Postgres for routing
...
containing its current label registry. The relay upserts those labels in Postgres:
...
Making the relay authoritative for label config would require the relay to survive
...
**Relay-authoritative registry with node sync:** labels are defined and stored in
the relay; the node pulls them on connect. Rejected because it makes the relay
a single point of failure for the label map and complicates node setup (how does the
...
- The relay's `path_labels` table is informational for routing. It must not be used
...
- Label names must be unique per user across all nodes (enforced at the relay).
