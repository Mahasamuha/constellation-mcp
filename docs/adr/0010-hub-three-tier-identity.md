# ADR 0010: Three-Tier OS Identity Resolution for Hub
...
The hub receives an RPC containing `user_oidc_sub` and OIDC claims from the relay.
Before spawning a subnode under the requesting user's OS identity, the hub must
resolve that OIDC identity to a local OS username. The resolution strategy must handle
LDAP/AD environments, direct mappings, and common OIDC claim patterns while defaulting
to a secure posture when resolution fails.
...
there is no fallback to the hub's own OS identity.
...
as a custom claim. The hub trusts the OIDC provider as authoritative for the claim
value.
...
No fallback to the hub's own identity. Silent fallback would mean a misconfigured
...
relay config, which defaults to forwarding all claims). It is the stable identity anchor
...
configuration on the hub host. Unusable at scale in LDAP environments. Rejected as
the default.
...
OS identity separation is the primary security guarantee of the hub model.

## Consequences

- The three-tier config is defined in the hub YAML (see `docs/hub.md` §2,
  "Identity resolution").
...
- Windows support (S4U2Self impersonation) is out of scope for the initial hub
  implementation. The three-tier resolution model applies unchanged; only the OS
