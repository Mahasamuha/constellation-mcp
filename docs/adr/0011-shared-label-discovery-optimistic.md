# ADR 0011: Hub Share Discovery Is Optimistic (Not Authoritative)
...
Users connected to a hub need a way to discover which shares are available
to them without requiring out-of-band coordination with the admin. The relay already
holds a synced copy of the hub's share registry (including permission config).
The question is whether discovery results should be guaranteed accurate or allowed to
be approximate.

## Decision

Hub share discovery is optimistic. The relay evaluates its synced copy of the
permission config to determine which shares to show a user. The result is not
authoritative — a share appearing in discovery may be rejected at dispatch if the hub's
...
**Visibility rule:** a share is included in discovery results if and only if the
relay's optimistic evaluation yields an access level other than `none`. Shares that
evaluate to `none` are hidden entirely — their existence is not revealed.

Discovery is backed by `HubShare` in Postgres, populated by the hub on
...
false positive — a share appears available but is rejected at dispatch. This mirrors how
NFS/Samba browse lists work and is acceptable.

The converse (a share the user can access not appearing in discovery) can occur when
Tier 1 grants access on a `default: none` share — Tier 1 resolution is hub-side only
...
- A user may see a share in `list_available_labels` that is subsequently rejected at
  dispatch. The dispatch error is informative (identity resolution failure or permission
  denied), not a protocol error.
- The admin share listing CLI (`constellation relay hub-shares list`, with backing
  `GET /api/admin/hub-shares` endpoint, admin-gated) provides a full view of the
  hub share registry including inaccessible shares. This is the primary
  troubleshooting tool for permission misconfigurations.
- Discovery results are served from `HubShare` in Postgres. Stale data (hub
  config changed between syncs) results in stale discovery — not a security issue since
  enforcement is always at the hub.
- The `HubShare` table stores the `permissionBlob` as JSON — the full share
